...
- **Globals.** Expressions read earlier steps by id, `input` (data passed to this execution), variables published by earlier runs in the session, `$errors.<stepId>` for recorded failures, and safe built-ins like `Math`, `JSON`, and `Date`. `new Date(...)` is the one constructor, and it gives the ISO 8601 string rather than a `Date` object, so every value in a plan stays plain JSON. The read-only `Date` API (`getUTCDate()`, `getTime()`, `toISOString()`, ...) works on that string, and `Date.UTC`/`Date.now`/`Date.parse` give milliseconds.
...
The website ships a playground that runs the three approaches side by side against the same live tools - callscript's one validated plan, the classic one-call-per-round-trip agent loop, and Code Mode's eval-the-script-for-real - with the system prompt each approach actually sends, the execution trace, and real token counts:

```sh
pnpm --filter web dev   # then open /playground
```

## Examples

Runnable tours, no build step (from `packages/callscript`):
...
- [`packages/callscript`](packages/callscript): the library
- [`apps/web`](apps/web): the landing page and playground (`pnpm --filter web dev`)
